...
app.post("/save-quiz-result", (req, res) => {
  ...
  const insertQuery =
    "INSERT INTO quiz_result (name, score, result) VALUES (?, ?, ?)";
...
app.get("/get-data-by-id", (req, res) => {
  const id = req.query.id; // Retrieve the ID from the query parameters

  const query = "SELECT * FROM quiz_result WHERE id = ?"; // Modify your SQL query to fetch data by ID
...
//   const updateQuery =
//     "UPDATE quiz_result SET name = ?, score = ?, result = ? WHERE id = ?";
